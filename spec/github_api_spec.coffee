describe "GithubApi", ->
  ...
  describe "#version", ->
    ...
    it "returns the version tag name", (done)->
      # make the request
      api.version (attributes)->
        expect(attributes.tag_name).toEqual("1.0.0-alpha.3")
        done()

      # return a json payload with the version tag name
    ...
    it "returns the latest release by major version", (done)->
      # make the request
      api.version (attributes)->
    ...
    it "returns the latest release by minor version", (done)->
      # make the request
      api.version (attributes)->
      ...
      jasmine.Ajax.requests.mostRecent().respondWith
        "status":       200
        "contentType":  'application/json'
        "responseText": '[{"tag_name": "3.1"}, {"tag_name": "3.16"}, {"tag_name": "3.10"}]'

    it "returns the latest release by patch version", (done)->
      # make the request
      api.version (attributes)->
    ...
    it "returns the latest release when an empty tag is present", (done)->
      # make the request
      api.version (attributes)->
        expect(attributes.tag_name).toEqual("1.11")
        done()
      ...
      jasmine.Ajax.requests.mostRecent().respondWith
        "status":       200
        "contentType":  'application/json'
        "responseText": '[{"tag_name": "1.1"}, {"tag_name": "1.11"}, {"tag_name": ""}]'
  ...
  xit "makes a requst for real", (done)->
    ...
    api.version (attributes)->
      console.log "#{api.user}/#{api.repo} v#{attributes.tag_name}, last published: #{attributes.published_at}"
      done()
